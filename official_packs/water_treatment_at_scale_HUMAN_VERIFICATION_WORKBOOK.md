...
**Update, 2026-07-18:** the 8 claims flagged for §9.3 mandatory human
confirmation (B2, D1, D2, D3, D4, D5, E3, F3) were re-audited with a
fresh re-read of every underlying source, ahead of the Founder's
confirmation pass - see
`water_treatment_at_scale_FOUNDER_CONFIRMATION_PACKET.md` for the full
claim-by-claim record. Three rows (D3, D4, E3) were narrowed as a
result - marked inline below - because the fresh re-read found the
sources support a more precise or more narrowly-attributed claim than
this Workbook's original wording. No claim failed outright.
...
| D3 | 4 | **Corrected 2026-07-18 (see Founder Confirmation Packet):** FRC has three distinct figures for three distinct points in the system - a distribution-point operational target (0.5-1.2 mg/L, condition-dependent), a point-of-delivery minimum (0.2 mg/L), and a household-consumption minimum (0.2 mg/L) - not one undifferentiated "0.2-0.5 mg/L" range | eCompendium T.6 (simpler 0.2-0.5 mg/L figure) + Oxfam WASH (three-tier breakdown) | Accessed (both) | Yes (both) | Verified, **NARROWED** on fresh re-read | **High** | **Yes** | See Founder Confirmation Packet, Claim D3 | Same as D1 |
| D4 | 4 | **Corrected 2026-07-18 (see Founder Confirmation Packet):** an upper limit on FRC (never exceed 4.0-5.0 mg/L) is stated by Oxfam WASH only - the eCompendium page does not itself state a numeric upper limit, only a general taste-rejection caution | Oxfam WASH only (not eCompendium, despite this row originally implying both) | Accessed | Yes | Verified, **NARROWED** on fresh re-read (single-source, not dual-source) | **High** | **Yes** | See Founder Confirmation Packet, Claim D4 | Same as D1 |
...
| E3 | 5 | **Corrected 2026-07-18 (see Founder Confirmation Packet):** shock chlorination at 50 mg/L is used for tank disinfection during operation/maintenance - the source states the figure and this context only; it does NOT itself state that this differs from routine dosing or that flushing is required before return to service (those were general-knowledge additions in the original row, not sourced) | eCompendium D.6 | Accessed | Yes | Verified (the 50 mg/L figure and O&M context only), **NARROWED** on fresh re-read | **High - toxic-substance handling threshold** | **Yes** | See Founder Confirmation Packet, Claim E3 - the "differs from routine dosing" comparison may only appear as the article's own general-knowledge reasoning (cross-referencing this pack's own D1-D4 routine-dosing figures), explicitly not attributed to this source | State qualitatively without attributing the flushing/comparison framing to this source: "a much higher chlorine concentration than routine drinking-water dosing is used for tank disinfection - confirm exact concentration and flush procedure with a qualified authority" |
